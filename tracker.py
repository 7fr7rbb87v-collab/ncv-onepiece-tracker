...
def should_exclude(title, exclude_words):
    ...
    words = [w.strip().lower() for w in clean(exclude_words).split(",") if w.strip()]
    return any(word in title_lower for word in words)


def ebay_search(product, token, marketplace_id):
    keywords = clean(product.get("keywords"))
    max_results = int(safe_float(product.get("max_results"), safe_float(os.getenv("EBAY_RESULTS_PER_PRODUCT"), 20)))
...
def convert_ebay_item(product, item, timestamp):
...
def grade_deal(total_price, market_value):
...
def add_market_math(listings):
    ...
    for product_id, items in by_product.items():
        totals = sorted([safe_float(item.get("total_price"), 0) for item in items if safe_float(item.get("total_price"), 0) > 0])
        ...
        for item in items:
            ...
            item.update({
                ...
                "estimated_spread": round(sale_target - buy_target, 2) if market_value else 0,
                "discount_to_market_percent": discount_to_market,
                "deal_grade": grade_deal(total_price, market_value),
            })
            output.append(item)

    return sorted(output, key=lambda row: (row.get("deal_grade") == "Pass", -safe_float(row.get("discount_to_market_percent"), 0)))


def update_history(existing_history, listings, timestamp):
    by_product = {}
    for item in listings:
        by_product.setdefault(item["product_id"], []).append(item)
    ...
    for product_id, items in by_product.items():
        ...
        existing_history.append({
            ...
            "market_value": market_value,
            "buy_target": round(market_value * 0.80, 2),
...
def build_retail_leads(timestamp):
...
def build_tiktok_videos(timestamp):
...
def run():
    ...
    timestamp = now_utc()
    watchlist = read_csv_if_exists("watchlist.csv", ["product_id", "product_name", "type", "era", "keywords", "exclude_words", "ebay_category", "condition_filter", "max_results"])
    ...
    if enable_ebay:
        if not client_id or not client_secret:
            ebay_status.update({"status": "missing_credentials", "message": "Add EBAY_CLIENT_ID and EBAY_CLIENT_SECRET as GitHub Secrets."})
        else:
            try:
